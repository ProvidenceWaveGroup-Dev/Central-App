...
export interface CO2MonitoringData {
  ...
  thresholds: {
    safe: number;      // < 800 ppm
    caution: number;   // 800–1200 ppm
    critical: number;  // > 1200 ppm
  };
  ...
  alerts: [
    {
      ...
      message: "CO\u2082 levels exceed safe threshold. Ventilation recommended.",
      timestamp: "Just now",
      acknowledged: false,
    },
  ],
...
export function CO2MonitoringCard() {
  ...
  // API-ready fetch function — replace internals with actual API call
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      // TODO: Replace with actual API call
      // const response = await fetch('/api/environmental/co2');
  ...
  const handleAcknowledgeAlert = (alertId: string) => {
    ...
    // TODO: POST to API to acknowledge alert
    // await fetch(`/api/environmental/co2/alerts/${alertId}/acknowledge`, { method: 'POST' });
  };
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white p-5">
      ...
      <div className="flex items-center justify-between mb-4 p-3 rounded-lg bg-gray-50 border border-gray-100">
        <div>
          ...
          <div className="text-2xl font-bold text-gray-900">
            {data.facilityAverage} <span className="text-sm font-normal text-gray-500">ppm</span>
          </div>
        </div>
            ...
            <div key={room.roomId} className="space-y-1.5">
              <div className="flex items-center justify-between text-sm">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-semibold ${cfg.bg} ${cfg.text}`}>
                    {cfg.label}
                  </span>
                </div>
              </div>
      ...
      {/* Historical Trend (Simplified Bar Chart) */}
      <div className="mb-4">
            ...
            .map((alert) => (
              <div key={alert.id} className="flex items-start gap-3 p-3 rounded-lg bg-red-50 border border-red-200">
                ...
                <button
                  ...
                  className="shrink-0 rounded-lg px-2.5 py-1 text-xs font-semibold bg-red-600 text-white hover:bg-red-700 transition-colors"
                >
                  Acknowledge
                </button>
              </div>
