...
export interface HumidityZoneReading {
  zoneId: string;
  zoneName: string;
  humidity: number; // %
  status: "dry" | "comfortable" | "high" | "mold_risk";
  moldRiskScore: number; // 0–100
  lastUpdated: string;
}

export interface HumidityHistoryEntry {
  timestamp: string;
  humidity: number;
}
...
export interface HumidityAlert {
  ...
  zoneName: string;
  humidity: number;
  type: "mold_risk" | "respiratory_concern" | "sustained_high" | "dry_air";
...
export interface HumidityMonitoringData {
  ...
  thresholds: {
    dry: number;          // < 30%
    comfortLow: number;   // 30%
    comfortHigh: number;  // 50%
    high: number;         // 50–60%
    moldRisk: number;     // > 60%
  };
  ...
  sustainedHighDuration: string | null; // e.g., "4h 30m"
  lastUpdated: string;
}
  ...
  alerts: [
    {
      ...
      type: "mold_risk",
      severity: "warning",
      message: "Sustained high humidity in Bathroom. Mold growth risk elevated. Consider dehumidification.",
      timestamp: "15 min ago",
      acknowledged: false,
    },
  ],
  sustainedHighDuration: "3h 45m",
...
export function HumidityRiskCard() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // const response = await fetch('/api/environmental/humidity');
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white p-5">
      ...
      {/* Average Humidity + Mold Risk Zones */}
      <div className="grid grid-cols-2 gap-3 mb-4">
        <div className="p-3 rounded-lg bg-gray-50 border border-gray-100">
          <div className="text-xs text-gray-500 mb-0.5">Average Humidity</div>
          <div className="text-2xl font-bold text-gray-900">
            {data.averageHumidity}<span className="text-sm font-normal text-gray-500">%</span>
          </div>
          <div className={`inline-flex rounded-full px-2 py-0.5 text-xs font-semibold mt-1 ${cfg.bg} ${cfg.text}`}>
            {cfg.label}
          </div>
        </div>
        <div className={`p-3 rounded-lg border ${data.moldRiskZones > 0 ? "bg-red-50 border-red-200" : "bg-green-50 border-green-200"}`}>
          ...
          <div className={`text-2xl font-bold ${data.moldRiskZones > 0 ? "text-red-600" : "text-green-600"}`}>
            {data.moldRiskZones}
          </div>
          <div className={`inline-flex rounded-full px-2 py-0.5 text-xs font-semibold mt-1 ${data.moldRiskZones > 0 ? "bg-red-100 text-red-700" : "bg-green-100 text-green-700"}`}>
            {data.moldRiskZones > 0 ? "Action Needed" : "All Clear"}
          </div>
        </div>
      </div>

      {/* Safe vs Risk Zone Visualization */}
      <div className="mb-4">
        ...
        <div className="relative h-4 rounded-full overflow-hidden">
          ...
          <div
            ...
            style={{ left: `${Math.min(data.averageHumidity, 100)}%` }}
          />
        </div>
      ...
      {data.sustainedHighDuration && data.overallStatus !== "comfortable" && data.overallStatus !== "dry" && (
        <div className="mb-4 p-3 rounded-lg bg-amber-50 border border-amber-200">
          ...
          <div className="text-xs text-amber-600">
            Elevated humidity has persisted for <span className="font-bold">{data.sustainedHighDuration}</span>.
            Prolonged exposure increases mold growth and respiratory health risks.
          </div>
        </div>
      )}

      {/* Per-Zone Readings with Mold Risk Score */}
            ...
            <div key={zone.zoneId} className="p-2.5 rounded-lg bg-gray-50 border border-gray-100">
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-semibold ${zcfg.bg} ${zcfg.text}`}>
                    {zcfg.label}
                  </span>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex-1 h-1.5 rounded-full bg-gray-200">
                  <div
                    className={`h-1.5 rounded-full transition-all duration-500 ${zone.moldRiskScore > 60 ? "bg-red-500" : zone.moldRiskScore > 30 ? "bg-amber-500" : "bg-green-500"}`}
                    style={{ width: `${zone.moldRiskScore}%` }}
                  />
                </div>
                <span className={`text-xs font-semibold w-8 text-right ${zone.moldRiskScore > 60 ? "text-red-600" : zone.moldRiskScore > 30 ? "text-amber-600" : "text-green-600"}`}>
                  {zone.moldRiskScore}%
                </span>
              </div>
            </div>
