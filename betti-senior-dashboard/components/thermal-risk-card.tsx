...
export interface ThermalZoneReading {
  zoneId: string;
  zoneName: string;
  temperature: number; // °F
  status: "cold_risk" | "comfortable" | "warm" | "heat_risk";
  lastUpdated: string;
}
...
export interface ThermalAlert {
  ...
  zoneName: string;
  temperature: number;
  type: "hypothermia_risk" | "heat_risk" | "rapid_change";
...
export interface ThermalMonitoringData {
  ...
  comfortRange: { min: number; max: number }; // °F
  thresholds: {
    coldRisk: number;   // < 65°F
    comfortLow: number; // 65°F
    comfortHigh: number;// 78°F
    heatRisk: number;   // > 82°F
  };
...
const statusConfig = {
  cold_risk: { label: "Cold Risk", bg: "bg-blue-100", text: "text-blue-700", bar: "bg-blue-500", icon: "text-blue-500" },
  comfortable: { label: "Comfortable", bg: "bg-green-100", text: "text-green-700", bar: "bg-green-500", icon: "text-green-500" },
  warm: { label: "Warm", bg: "bg-amber-100", text: "text-amber-700", bar: "bg-amber-500", icon: "text-amber-500" },
  heat_risk: { label: "Heat Risk", bg: "bg-red-100", text: "text-red-700", bar: "bg-red-500", icon: "text-red-500" },
};
...
export function ThermalRiskCard() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // const response = await fetch('/api/environmental/thermal');
  ...
  // Map temperature to a position on the comfort gauge (50°F to 95°F range)
  const gaugeMin = 50;
  const gaugeMax = 95;
      ...
      <div className="flex items-center justify-between mb-4 p-3 rounded-lg bg-gray-50 border border-gray-100">
        <div>
          ...
          <div className="text-2xl font-bold text-gray-900">
            {data.averageTemp}°<span className="text-sm font-normal text-gray-500">F</span>
          </div>
        </div>
        <div className="text-right">
          ...
          <div className="text-xs text-gray-400 mt-1">
            Comfort: {data.comfortRange.min}°–{data.comfortRange.max}°F
          </div>
        </div>
      </div>
      ...
      <div className="mb-4">
        ...
        <div className="relative h-4 rounded-full overflow-hidden">
          ...
          <div
            ...
            style={{ left: `${Math.min(Math.max(tempPct, 0), 100)}%` }}
          />
        </div>
              ...
              <div key={alert.id} className={`flex items-start gap-3 p-3 rounded-lg border ${alert.severity === "critical" ? "bg-red-50 border-red-200" : "bg-amber-50 border-amber-200"}`}>
                ...
                <div className="flex-1 min-w-0">
                  <div className={`text-sm font-medium ${alert.severity === "critical" ? "text-red-700" : "text-amber-700"}`}>
                    {alert.zoneName} — {alert.temperature}°F
                  </div>
